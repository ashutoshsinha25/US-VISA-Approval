...
from visa_approval.configuration.aws_connector import S3Client 
...
class SimpleStorageService:
    ...
    def s3_key_path_available(self, bucket_name, s3_key):
        try:
            ...
            file_objs = [file_obj for file_obj in bucket.objects.filter(Prefix=s3_key)]
            if len(file_objs) > 0:
                return True 
            else:
                return False
    ...
    def get_df_from_object(self, object_: object) -> pd.DataFrame:
        ...
        try:
            ...
            df = self.read_csv(content, na_values="na")
